...
class DSBranch(nn.Layer):
    ...
    # l6是瓶颈块，输出1024
        ...
        self.conv_smooth19 = ConvBNRelu(19, 128, 3)

        self.relu = nn.ReLU()
        self.spp = SDAPPM(1024, 112, 256)    # 将128改为112？
        #self.arm = ARModule(384, 128)
        #  OCRBlock
        self.ocrblock = OCRHead(19, 384)
    ...
    def forward(self, x):
        ...
        out_8 = self.conv_smooth19(out_8)
    # smooth：相当于整合
    #     return out_8
...
class AlignedModule(nn.Layer):
    ...
    def flow_warp(self, input, flow, size):    # H，flow,size
    ...
    def forward(self, low_feature, h_feature):
        ...
        low_feature = self.down_l(low_feature)  # 2，64，64，128
        ...
        _att = 1 - att
        low_feature = low_feature * _att

        low_feature = self.upconv1(low_feature)
        ##############################################
...
class OCRHead(nn.Layer):
    def __init__(self,
        ...
        # 这里conv3x3_ocr就是图中的蓝色块里的东西，对应论文中的pixel reprations，深层特征
        self.conv3x3_ocr = layers.ConvBNReLU(
            256, ocr_mid_channels, 3, padding=1)
        self.cls_head = nn.Conv2D(ocr_mid_channels, self.num_classes, 1)
        # 粉色框，浅层特征
        self.aux_head = nn.Sequential(
            layers.ConvBNReLU(128,
                              128, 1),
            nn.Conv2D(128, self.num_classes, 1))

        self.init_weight()

    def forward(self, feat_shallow, feat_deep): # 128  256
        # feat_shallow, feat_deep = feat_list[self.indices[0]], feat_list[
        #     self.indices[1]]
        feat_shallow = F.interpolate(feat_shallow, size=feat_deep.shape[2:], mode='bilinear', align_corners=True) # 128
        # feat_deep = F.interpolate(feat_deep, size=feat_shallow.shape[2:], mode='bilinear', align_corners=True)  # 256
        soft_regions = self.aux_head(feat_shallow)
        # print(soft_regions.shape)
        pixels = self.conv3x3_ocr(feat_deep)
        object_regions = self.spatial_gather(pixels, soft_regions)
        ocr = self.spatial_ocr(pixels, object_regions)  # [2, 512, 16, 32]
