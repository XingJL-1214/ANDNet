...
class STDCNet(nn.Layer):
    ...
    def __init__(self,
        ...
        if (layers == [4, 5, 3]):  #stdc1446
            ...
            self.x16 = nn.Sequential(self.features[6:11])  # 6-7-8-9-10
            self.x32 = nn.Sequential(self.features[11:])  # 11
        elif (layers == [2, 2, 2]):  #stdc813
            self.x2 = nn.Sequential(self.features[:1])  # 0
            self.x4 = nn.Sequential(self.features[1:2])  # 1
            self.x8 = nn.Sequential(self.features[2:4])  # 2 -3
            self.x16 = nn.Sequential(self.features[4:6]) # 4-5
            self.x32 = nn.Sequential(self.features[6:])  # 6
        else:
