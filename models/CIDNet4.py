...
class CIDNet4(nn.Layer):
    def __init__(self,
        ...
        self.aux_head2 = SegHead(128, 128, num_classes)
        self.aux_head3 = SegHead(128, 128, num_classes)
...
class DSBranch(nn.Layer):
    ...
    # l6是瓶颈块，输出1024
        ...
        ###### 新代码 ##################
        self.conv_head32 = ConvBNRelu(128, 128, 3)
        ...
        # 消融实验变换通道数
        self.conv_c = nn.Conv2D(in_channels=1024, out_channels=128, kernel_size=3, stride=1, padding=1, bias_attr=False)
        self.conv_c1 = nn.Conv2D(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1, bias_attr=False)
        self.conv_c2 = nn.Conv2D(in_channels=512, out_channels=128, kernel_size=3, stride=1, padding=1, bias_attr=False)
        self.conv_c3 = nn.Conv2D(in_channels=128, out_channels=512, kernel_size=3, stride=1, padding=1, bias_attr=False)
        self.conv_c4 = nn.Conv2D(in_channels=256, out_channels=128, kernel_size=3, stride=1, padding=1, bias_attr=False)
    # pagFM: DSIM
    ...
    def forward(self, x):
        ...
        l6 = self.l6(l5)
        l6 = F.interpolate(l6, size=h5.shape[2:], mode='bilinear', align_corners=True)
        l6 = self.conv_c(l6)
        l7 = self.conv_head32(l6)
        out_8 = h5 + l6
        #  ##############baseline###############################################
        ...
        # # # # ##################消融实验3#####################################
        # l4 = self.conv_c4(l4)
        # l4 = F.interpolate(l4, size=h4.shape[2:], mode='bilinear', align_corners=True)
        # # l4 = self.conv_head16(l4)
        # h4_ = h4 + l4   # c 128
        # h5 = self.h5(self.relu(h4_))  # c ?-->128,  s 64×128
        # l5 = self.conv_c3(h4_)    # c 128-->512,  s 64×128
        # # ##################消融实验3#####################################
        #
        # h5 = self.h5(self.relu(h4_))    # c ?-->128,  s 64×128
        ...
        # # ##################消融实验2#####################################
        # l5 = self.conv_c2(l5)       # c 512--->128
        # l5 = F.interpolate(l5, size=h5.shape[2:], mode='bilinear', align_corners=True)
        # l5_ = h5 + l5       # c 128
        # h5_ = self.conv_c3(l5_)   # 2, 512, 64, 128

        # # ##################消融实验2####################################
        # l6 = self.l6(self.relu(h5_))    # c 512-->1024, s 16×32
        #
        # # ###################消融实验1#####################################
        # l6 = F.interpolate(l6, size=h5_.shape[2:], mode='bilinear', align_corners=True)
        # l6 = self.conv_c(l6)  # l6-[24, 128, 64, 128]
        # l7 = self.conv_head32(l6)  # l7-[24, 128, 64, 128]
        # l8 = self.conv_c1(l7)  # l8-[24, 256, 64, 128]
        # # l7降通道
        # out_8 = l7 + l5_
    ...
    #     return out_8
        ...
        return out_8, h4, h5, l7
        # return out_8, h4_, h5_, l8
