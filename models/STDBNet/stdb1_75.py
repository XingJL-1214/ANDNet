...
class DSBranch(nn.Layer):
    ...
    def forward(self, x):
        ...
        # h4_ = h4 + F.interpolate(
        #     self.compression4(l4),
        ...
        atten = self.arm(h5, l6)
        l6_ = self.conv_head32(l6)
        feat_32 = paddle.multiply(l6_, atten) + l6_
        feat_32_up = F.interpolate(feat_32, size=[height_output, width_output], mode='bilinear')

        feat_8 = paddle.multiply(h5, (1-atten)) + h5
        out_8 = feat_32_up + feat_8
        out_8 = self.conv_smooth128(out_8)

        return out_8, h4, h5, l6
