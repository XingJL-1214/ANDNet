...
class ConvBNHSwish(nn.Layer):
    ...
    def forward(self, x):
        out = self.hardswish(self.bn(self.conv(x)))
        return out
...
class DSBranch(nn.Layer):
    ...
    def __init__(self, pretrained=None):
        ...
        # )
        self.down5 = nn.Sequential(
            nn.Conv2D(128, 256, kernel_size=1, stride=2, bias_attr=False),
            SyncBatchNorm(256, data_format='NCHW'),
            nn.ReLU(),
            nn.Conv2D(256, 512, kernel_size=1, stride=2, bias_attr=False),
            SyncBatchNorm(512, data_format='NCHW'),
        )
        ...
        self.cw1 = ChannelWise2(384, 256)
        self.cw2 = ChannelWise2(640, 512)
    ...
    def forward(self, x):
        ...
        # l4_ = self.cw1(paddle.concat([l4, self.down4(h4)], axis=1))
        # h4_ = self.pag1(h4, l4)
        h4_ = self.pag1(h4, self.compression4(l4))
        h5 = self.h5(self.relu(h4_))
        l5 = self.l5(self.relu(l4_))

        l5_ = l5 + self.down5(h5)
        # h5_ = self.pag2(h5, l5)
        # l5_ = self.cw2(paddle.concat([l5, self.down5(h5)], axis=1))
        h5_ = self.pag2(h5, self.compression5(l5))

        l6 = self.l6(self.relu(l5_))
        l6_= F.interpolate(l6, size=[height_output, width_output], mode='bilinear')
        out = h5_ + l6_
        out = self.conv_smooth128(out)
...
class ChannelWise2(nn.Layer):
    def __init__(self, channel, outchannel, reduction=4):
        super(ChannelWise2, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2D(1)
        self.max_pool = nn.AdaptiveMaxPool2D(1)
        self.conv_pool = nn.Sequential(
            ConvBNRelu(channel, channel//reduction, 1, 1),
            Conv_BN(channel // reduction, channel, 1, 1))
        self.sigmoid = nn.Sigmoid()
        self.tanh = nn.Tanh()
        self.out_conv = Conv_BN(channel, outchannel, 1, 1)

    def forward(self, x):
        y_a = self.avg_pool(x)
        y_m = self.max_pool(x)
        y_a = self.conv_pool(y_a)
        y_m = self.conv_pool(y_m)
        y = y_a + y_m
        y = self.tanh(y)
        out = x * y
...
class ARModule(nn.Layer):
    def __init__(self, in_chan, out_chan, *args, **kwargs):
        ...
        self.conv_hard = ConvBNHSwish(in_chan, out_chan, kernel=1, stride=1)
